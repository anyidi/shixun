...
def get_baidu_credentials():
    return os.getenv('BAIDU_APP_ID', ''), os.getenv('BAIDU_SECRET_KEY', '')
...
class BaiduTranslator(CommonTranslator):
    ...
    def get_url(self, from_lang, to_lang, query_text):
        ...
        sign = m1.hexdigest()
        # 拼接URL
        url = API_URL +'?appid=' + self.app_id + '&q=' + urllib.parse.quote(query_text) + '&from=' + from_lang + '&to=' + to_lang + '&salt=' + str(salt) + '&sign=' + sign
        return url
